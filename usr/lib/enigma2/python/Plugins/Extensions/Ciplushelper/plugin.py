...
class Ciplushelper(Screen):
    ...
    def __init__(self, session):
        ...
        model = ""
        if exists(
                "/usr/lib/enigma2/python/Plugins/Extensions/Ciplushelper/info.txt"):
            try:
        ...
        if model:
            ...
            if "ciplushelper-arm" in model or "ciplushelper-zgemma-arm" in model:
                if not exists("/etc/cicert.bin"):
                    menu_list.append(
                        (_("Install") + " " + _("version from Zgemma"), "install_cicert_bin"))
                else:
                    ...
                    menu_list.append(
                        (_("Install") + " " + _("default version"), "install_default"))
        ...
        if exists("/etc/ciplus/customer.pem") and exists("/etc/ciplus/device.pem") and exists(
                "/etc/ciplus/root.pem") and exists("/etc/ciplus/param"):
    ...
    def run(self):
        ...
        ) and self["menu"].l.getCurrentSelection()[1]
        ...
        if returnValue == "install_cicert_bin":
            ...
            os_system(
                "cp {}/ciplushelper_bin/zgemma-arm/ciplushelper /usr/bin/ciplushelper && chmod 755 /usr/bin/ciplushelper".format(plugin_path))
        ...
        if returnValue == "install_default":
            ...
            os_system(
                "cp {}/ciplushelper_bin/arm/ciplushelper /usr/bin/ciplushelper && chmod 755 /usr/bin/ciplushelper".format(plugin_path))
